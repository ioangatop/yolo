...
class ProcessDetections:
    ...
    def __init__(self, box_mode='xywh'):
        self.box_mode = box_mode
        self.labels = ['person', 'head']
    ...
    def _unpack_response(self, response, trt_output_names):
        input_shape, target_shape, pred = [
            response.as_numpy(name) for name in trt_output_names
        ]
    ...
    def forward(self, response, trt_output_names, threshold=0.3, iou_threshold=0.5, class_id=None, **kwargs):
        input_shape, target_shape, pred = self._unpack_response(response, trt_output_names)
    ...
    def __call__(self, response, trt_output_names, threshold=None, iou_threshold=None, class_id=None, **kwargs):
        return self.forward(response, trt_output_names, threshold, iou_threshold, class_id, **kwargs)
