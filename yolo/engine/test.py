...
def test(data,
    ...
    if training:  # called by train.py
        ...
        model = build_model(args.cfg, weights, nc=int(data_dict['nc']), device=device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-cfg', default='/workdir/models/COCO-Detection/yolov4-p5.yaml')
    ...
    parser.add_argument('--gt-coco-file', default=None, help='Ground-truth COCO annotations')
    parser.add_argument('--save-txt', default=False, help='save results to *.txt')
    parser.add_argument('--save-json', default=True, help='save results to *.json COCO format')
    parser.add_argument('--no-verbose', action='store_false', help='Not to report mAP by class')
    opt = parser.parse_args()

    if opt.task in ['val', 'test']:  # run normally
        device = select_device(opt.device)
        data_cfg = load_data_cfg(opt.data_cfg)
        model = build_model(opt.model_cfg, opt.model_weights, nc=int(data_cfg['nc']), device=device)
